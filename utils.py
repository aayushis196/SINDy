...
def evaluate_model_plot(model, traj, norm_tr, coefficient_mask=None):
    ...
    for i, action_i in enumerate(actions):
        next_state = model(prev_state, action_i.unsqueeze(0),coefficient_mask)
        pred_states_multistep.append(next_state.squeeze(0))
        prev_state = next_state

    pred_states_singlestep = model(states_norm[:-1], actions,coefficient_mask)
